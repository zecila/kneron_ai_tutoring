...
class _FakeStreamResponse:
    ...
    def iter_lines(self):
        return [json.dumps(event).encode() for event in self._events]

    def close(self):
        self.closed = True


def _make_wav(frames=b"\x01\x00" * 3):
    output = io.BytesIO()
...
class TTSBridgeTest(unittest.TestCase):
    ...
    def test_api_tts_preserves_busy_status_for_browser_retry(self):
        with patch.object(
            server,
            "_synthesize_tts_wav",
            side_effect=server.TTSServiceError("TTS service is busy; please retry", status_code=503),
        ):
            with server.app.test_client() as client:
                response = client.post("/api/tts", json={"text": "Hello"})

        self.assertEqual(response.status_code, 503)
        self.assertEqual(response.get_json(), {"error": "TTS service is busy; please retry"})
    ...
    def test_tts_wav_chunks_are_merged_into_one_valid_wav(self):
        ...
        merged = server._merge_tts_wav_chunks([
            _make_wav(first_frames),
            _make_wav(second_frames),
        ])
    ...
    def test_tts_base64_stream_returns_valid_wav(self):
        wav_bytes = _make_wav()
        response = _FakeStreamResponse([
            {"event": "chunk", "data": base64.b64encode(wav_bytes).decode()},
            {"event": "done", "data": ""},
        ])

        self.assertEqual(server._parse_tts_base64_stream(response), wav_bytes)

    def test_tts_base64_stream_rejects_truncated_response(self):
        response = _FakeStreamResponse([
            {"event": "chunk", "data": base64.b64encode(_make_wav()).decode()},
        ])

        with self.assertRaisesRegex(server.TTSServiceError, "before completion"):
            server._parse_tts_base64_stream(response)

    def test_tts_request_uses_reliable_base64_stream(self):
        wav_bytes = _make_wav()
        response = _FakeStreamResponse([
            {"event": "chunk", "data": base64.b64encode(wav_bytes).decode()},
            {"event": "done", "data": ""},
        ...
        with patch.object(server.requests, "post", return_value=response) as post:
            result = server._request_tts_wav_chunk("Hello", "model", "voice", 1, 1)

        self.assertEqual(result, wav_bytes)
        self.assertTrue(response.closed)
        post.assert_called_once_with(
            f"{server.TTS_BASE}/audio/tts",
            json={
                "text": "Hello",
            ...
            stream=True,
            timeout=server.TTS_REQUEST_TIMEOUT,
        )

    def test_tts_request_recovers_from_transient_event_error(self):
        failed_response = _FakeStreamResponse([
            {"event": "error", "data": "temporary capacity error"},
        ])
        wav_bytes = _make_wav()
        successful_response = _FakeStreamResponse([
            {"event": "chunk", "data": base64.b64encode(wav_bytes).decode()},
            {"event": "done", "data": ""},
        ])

        with patch.object(server.requests, "post", side_effect=[failed_response, successful_response]):
            with patch.object(server, "_recover_tts_model") as recover:
                with patch.object(server.time, "sleep") as sleep:
                    result = server._request_tts_wav_chunk("Hello", "model", "voice", 1, 1)

        self.assertEqual(result, wav_bytes)
        recover.assert_called_once_with("model", "voice")
        sleep.assert_called_once_with(server.TTS_RETRY_BASE_DELAY)
        self.assertTrue(failed_response.closed)
        self.assertTrue(successful_response.closed)

    def test_tts_initialization_waits_for_healthy_instance(self):
        init_response = _FakeJSONResponse({"status": "starting"})
        starting_response = _FakeJSONResponse({
            "instances": [{"model_name": "model", "version": "voice", "status": "starting"}]
        })
        healthy_response = _FakeJSONResponse({
            "instances": [{"model_name": "model", "version": "voice", "status": "healthy"}]
        })

        with patch.object(server.requests, "post", return_value=init_response):
            with patch.object(server.requests, "get", side_effect=[starting_response, healthy_response]):
                with patch.object(server.time, "sleep"):
                    server._initialize_tts_model("model", "voice", wait_timeout=5)
    ...
    def test_avatar_health_reports_unavailable_without_exposing_upstream_details(self):
        with patch.object(server, "_livetalking_is_ready", side_effect=server.requests.Timeout("timed out")):
            with server.app.test_client() as client:
    ...
    def test_avatar_offer_fails_fast_when_livetalking_is_not_ready(self):
        with patch.object(server, "resolve_lesson_access", return_value=True):
            with patch.object(server, "_livetalking_is_ready", return_value=False):
                with patch.object(server.requests, "post") as post:
                    with server.app.test_client() as client:
                        response = client.post(
                            "/api/lessons/lesson-1/avatar/webrtc/offer",
                            json={"type": "offer", "sdp": "valid-sdp"},
                        )
